...
vk.setOptions({
    token: '67c0405bd0147667cbae6c23f2ca7c94455ebe77c45597b579519ba78a3292235645acaa90b3469e5e1b8', // токен группы
    apiMode: 'parallel',
	pollingGroupId: 181362707 // 1 замени на id группы 
});
...
vk.updates.hear(/^(?:кикнуть)(\s?https\:\/\/vk\.com\/)?(id)?([0-9]+)?([^]+)?/i, (message) => { 
	...
	if(message.$match[4]) { 
		...
		if(message.$match[3] == 232357251) return message.reply('⚠ ➾ Отказ'); 
...
///////////////////////////////////////////////////////////////////////////
	vk.updates.hear(/^(?:бот)$/i, (message) => {
		...
		return message.send(`
			📝 ➾ Проект: ${config.bot}
			💻 ➾ Версия: 2.1
			💀 ➾ Создатель: https://vk.com/mix_fix_sip 
	...
	vk.updates.hear(/^(?:беседы)$/i,  (message) => { 
		return message.send(`
			...
			https://vk.me/join/AJQ1d8qQTQ3tzcXn1veG7oc8
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 	vk.updates.hear(/^(?:кейс|бонус)/i, (message) => {  
 		if(message.$from.type != 'user') return message.send(`Брать бонус можно только в ЛС {у БОТА}`);
		let user = acc.users[user_id(message.user)];
 		let id = user_id(message.user)
